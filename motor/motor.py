...
def lamina_punto(materia, k, titulo, detalle, out):
    ...
    f_d, l_d = bloque(d, detalle, F_BODY, 50, W - 180, 5, 34)
    alto = len(l_t) * f_t.size * 1.12 + 50 + len(l_d) * f_d.size * 1.3
    y = H * 0.5 - alto / 2
...
def main(pj, salida):
    ...
    for k, q in enumerate(p["puntos"], start=1):
        lamina_punto(m, k, q["t"], q["d"], f"_e/l{k}.png")
    ...
    sh(f'ffmpeg -y -hide_banner -loglevel error -i _e/video.mp4 -i "{p["voz"]}"{amb} -filter_complex "{filtro}" '
       f'-map 0:v -map "[a]" -c:v copy -c:a aac -b:a 160k -shortest -movflags +faststart "{salida}"')
